...
public class ClustersGraph extends ApplicationFrame {
	...
	private static final String FILE_NAME = "week_in_intervals.arff";
//	private static final String FILE_NAME = "week_out_intervals.arff";
	static String Week_File = "Vweek_noid_In2.csv";
//	static String Week_File = "Vweek_noid_Out2.csv";
	...
	private static XYDataset createDataset() throws Exception 
	{
		...
		double[] timeInIntervals = visualizationTools.Intervals.timeIntervalsToDouble(visualizationTools.Intervals.getStartTimeIntervals());
		...
		for (int i = 0; i < 24; i++){
			...
			if (a4 < 0) { a4 = 0; }
			a5 = mean5[i][0]-mean5[i][1];
			if (a5 < 0) { a5 = 0; }

			cluster0.add(timeInIntervals[i+1]/2.+timeInIntervals[i]/2., 
					mean0[i][0]/(timeInIntervals[i+1]-timeInIntervals[i]),
					a0/(timeInIntervals[i+1]-timeInIntervals[i]),
					mean0[i][0]+mean0[i][1]/(timeInIntervals[i+1]-timeInIntervals[i]));
			cluster1.add(timeInIntervals[i+1]/2.+timeInIntervals[i]/2., 
					mean1[i][0]/(timeInIntervals[i+1]-timeInIntervals[i]),
					a1/(timeInIntervals[i+1]-timeInIntervals[i]), 
					mean1[i][0]+mean1[i][1]/(timeInIntervals[i+1]-timeInIntervals[i]));
			cluster2.add(timeInIntervals[i+1]/2.+timeInIntervals[i]/2., 
					mean2[i][0]/(timeInIntervals[i+1]-timeInIntervals[i]),
					a2/(timeInIntervals[i+1]-timeInIntervals[i]), 
					mean2[i][0]+mean2[i][1]/(timeInIntervals[i+1]-timeInIntervals[i]));
			cluster3.add(timeInIntervals[i+1]/2.+timeInIntervals[i]/2.,
					mean3[i][0]/(timeInIntervals[i+1]-timeInIntervals[i]),
					a3/(timeInIntervals[i+1]-timeInIntervals[i]),
					mean3[i][0]+mean3[i][1]/(timeInIntervals[i+1]-timeInIntervals[i]));
			cluster4.add(timeInIntervals[i+1]/2.+timeInIntervals[i]/2., 
					mean4[i][0]/(timeInIntervals[i+1]-timeInIntervals[i]),
					a4/(timeInIntervals[i+1]-timeInIntervals[i]), 
					mean4[i][0]+mean4[i][1]/(timeInIntervals[i+1]-timeInIntervals[i]));
			cluster5.add(timeInIntervals[i+1]/2.+timeInIntervals[i]/2., 
					mean5[i][0]/(timeInIntervals[i+1]-timeInIntervals[i]), 
					a5/(timeInIntervals[i+1]-timeInIntervals[i]), 
					mean5[i][0]+mean5[i][1]/(timeInIntervals[i+1]-timeInIntervals[i]));
		}
	...
	{
		visualizationTools.Intervals.getTimeIntervalsStart("week(copie).csv",24);
		try {
